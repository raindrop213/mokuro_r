...
def crop_and_split_image(input_path, output_folder, crop_edges):
    ...
    with Image.open(input_path) as img:
        ...
        if cropped_width > cropped_height:
            ...
            left_path = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(input_path))[0]}_b{os.path.splitext(input_path)[1]}")
            left_image.save(left_path, quality=95)

            # 裁剪图片的右半部分
            right_image = img_cropped.crop((new_width, 0, cropped_width, cropped_height))
            right_path = os.path.join(output_folder, f"{os.path.splitext(os.path.basename(input_path))[0]}_a{os.path.splitext(input_path)[1]}")
...
# 设置文件夹路径
input_folder = r'F:\qBit\manga\[押見修造] おかえりアリス\OkaeriArisu v04'
output_folder = input_folder + "a"
os.makedirs(output_folder, exist_ok=True)

# 批量裁剪和分割图片 (left, top, right, bottom)。
crop_edges = (478, 0, 478, 0)
